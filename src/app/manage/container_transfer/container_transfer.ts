...
@Component({
  standalone: true,
  selector: 'app-container_transfer',
  imports: [ScanModal, FormsModule],
  templateUrl: './container_transfer.html',
  styleUrl: './container_transfer.scss',
})

export class ContainerTransfer implements OnInit, AfterViewInit {
  table: any
  pageLength = 50
  inputA01?: string
  inputA02?: string
  ...
  submit() {
    SweetAlert2Helper.Confirm('確認要載體移轉嗎?', () => {
      if (!this.inputA01 || !this.inputA02) {
        SweetAlert2Helper.Alert('請輸入載體和儲位')
        return
      }
      ...
      this.containerService.updateIMGS03(container, imgs03).then(result => {
        if (result.succ) {
          SweetAlert2Helper.AlertSucc('載體移轉成功', () => {
            this.clear()
          })
        } else {
