...
@Component({
  standalone: true,
  selector: 'app-container_change',
  imports: [ScanModal, FormsModule],
  templateUrl: './container_change.html',
  styleUrl: './container_change.scss',
})

export class ContainerChange implements OnInit, AfterViewInit {
  table: any
  pageLength = 50
  inputA01?: string
  inputA02?: string
  ...
  submit() {
    SweetAlert2Helper.Confirm('確認要載體更換嗎?', () => {
      ...
      this.containerService.changeContainer(from_cotainer, to_container).then(result => {
        if (result.succ) {
          SweetAlert2Helper.AlertSucc('載體更換成功', () => {
            this.clear()
          })
        } else {
