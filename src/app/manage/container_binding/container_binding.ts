...
@Component({
  standalone: true,
  selector: 'app-container_binding',
  imports: [ScanModal, FormsModule],
  templateUrl: './container_binding.html',
  styleUrl: './container_binding.scss',
})

export class ContainerBinding implements OnInit, AfterViewInit {
  table: any
  pageLength = 50
  inputA01?: string
  inputA02?: string
  ...
  submit() {
    SweetAlert2Helper.Confirm('確認要綁定嗎?', () => {
      ...
      this.containerService.updateIMGS03(this.inputA01, this.inputA02).then(result => {
        if (result.succ) {
          SweetAlert2Helper.AlertSucc('載體綁定成功', () => {
            this.clear()
          })
        } else {
